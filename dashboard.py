    ...
    with _chart_cols[1]:
        ...
        def _render_comparison_chart() -> None:
            ...
            with st.container(border=True, key="chart_card_comparison"):
                ...
                # בלי הסבר, האחוזים בקצה הקווים לא ברורים - מה זה "תשואה" כאן,
                # ולמה זה לא סתם שווי התיק (שהיה קופץ בכל קנייה חדשה, 14.9.2026
                # "גם התיק משתנה כל הזמן, אז מה ההיגיון").
                st.caption(
                    "ממוצע משוקלל (לפי סכום השקעה) של תשואת כל אחזקה מול מחיר הכניסה שלה - "
                    "לא שווי התיק הכולל, כדי שקנייה חדשה לא תיראה כמו קפיצת תשואה."
                )
