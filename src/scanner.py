...
def compute_holdings_value_by_currency(conn, price_fetcher=None) -> dict[str, float]:
    """שווי שוק נוכחי של כל האחזקות הפתוחות (bought=1), לפי מטבע - לתצוגה
    בלבד ("שווי התיק הכולל" בדשבורד, ליד שדה "גודל התיק הכולל" הידני). מחושב
    תמיד מחדש מהמחירים החיים, לא נשמר ב-config. price_fetcher אופציונלי -
    הדשבורד מעביר את הגרסה שלו עם cache (st.cache_data)."""
...
def _scan_one_index(
    ...
    # 22.9.2026 ניסינו גם לחסום לפי prev_close_gap (הבסיס-להשוואה, לא הסגירה
    # האחרונה עצמה, ישן מדי) - בכוונה טובה (תוקן אז מקרה קיצון אמיתי, פער של
    # 5 ימים), אבל prev_close_gap נדלק גם על פער יום-מסחר אחד רגיל (לא לוקח
    # חגים בחשבון - ר' market_data._expected_last_close_date), אז בפועל זה
    # חסם כמעט את *כל* הסריקה/ההתראות לאורך כל תקופת הפיגור הרחבה של yfinance
    # (17-23.9.2026) - signal_log צנח מ-86 אותות ביום ל-0 לגמרי (19-21.9),
    # לא רק המקרים הבעייתיים שהתיקון התכוון לתפוס (24.9.2026, "איך זה שאין
    # התראות היום"). last_close_date עצמו (is_data_stale למעלה) כבר תופס את
    # המקרה הקריטי באמת - מחיר חי/אחרון שהוא עצמו ישן - בלי התלות-יתר בבסיס
    # ההשוואה. תצוגת "שינוי יומי" בדשבורד עדיין מציגה אזהרת פער בנפרד (ר'
    # prev_close_gap שם) - זה לא השתנה, רק ההחלטה אם *להתריע בכלל*.
